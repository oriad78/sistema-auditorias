    ...
    for _, row in steps.iterrows():
        with st.expander(f"🔹 {row['step_code']} - {row['description']} ({row['status']})"):
            ...
            if st.button("💾 Guardar y Firmar", key=f"btn_{row['id']}"):
                ...
                conn.commit()
                registrar_log(row['id'], st.session_state.user_name, "Actualizó notas.")
                st.toast("Guardado")
    conn.close()
...
def vista_login():
    st.title("⚖️ AuditPro")
    tab1, tab2, tab3 = st.tabs(["Ingresar", "Registrarse", "Recuperar Clave"])
    ...
    with tab2:
        ...
        r_em = st.text_input("Correo Electrónico")
        r_pw1 = st.text_input("Contraseña", type="password")
        r_pw2 = st.text_input("Repetir Contraseña", type="password")
    ...
    with tab3:
        ...
        f_em = st.text_input("Ingresa tu correo registrado")
        f_nm = st.text_input("Ingresa tu nombre completo")
        f_pwnew = st.text_input("Nueva Contraseña", type="password")
        if st.button("Actualizar Contraseña"):
            conn = get_db_connection()
            user = conn.execute("SELECT id FROM users WHERE email=? AND full_name=?", (f_em, f_nm)).fetchone()
            if user:
