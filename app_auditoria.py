...
def seccion_materialidad(client_id):
    ...
    with st.container(border=True):
        ...
        base = col1.number_input("Base", value=mat_data[0] if mat_data else 0.0)
            ...
            for _, row in pasos.iterrows():
                ...
                with c_file:
                    ...
                    for fid, fname, fdata in archivos:
                        st.download_button(f"📥 {fname}", data=fdata, file_name=fname, key=f"dl_{fid}")
    conn.close()
...
def vista_login():
    ...
    with t2:
        with st.form("registro"):
            n, e, p = st.text_input("Nombre"), st.text_input("Correo"), st.text_input("Contraseña", type="password")
            if st.form_submit_button("Crear"):
                conn = get_db_connection()
                try: conn.execute("INSERT INTO users (full_name, email, password_hash) VALUES (?,?,?)", (n, e, hash_pass(p))); conn.commit(); st.success("Ok")
...
def vista_principal():
    with st.sidebar:
        ...
        cn, ct = st.text_input("Empresa"), st.text_input("NIT")
